...
void OnRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    ...
    if (size <= BUFFER_SIZE)
    {
      ...
      String InfoPacket = "received packet in hex:\n";
      for (uint16_t i = 0; i < size; i++) {
          char hexString[3]; // Platz für 2 hexadezimale Ziffern und das Nullterminierungszeichen
          sprintf(hexString, "%02X", payload[i]);
          InfoPacket += hexString;
      }
      InfoPacket += "\nwith rssi " + String(rssi) + ", length " + String(rxSize) + ", " + String(packetcount);
      bool match = true;
      ...
      if (match) {
        ...
        factory_display.drawString(0, 10, (String)currentPKwh);
        factory_display.drawString(0, 20, InfoPacket);
        factory_display.display();
