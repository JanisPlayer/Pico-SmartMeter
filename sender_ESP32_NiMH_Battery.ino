...
void setup() {
  ...
  if (useLoRaMode == true) {
    ...
    //delay(1000);
    Serial.print(F("Initialisierung der Hardware-Seriellen Schnittstelle "));
    //delay(2000);
    //digitalWrite(VEXT_CTRL, HIGH);
    SmlSerial.begin(9600, SERIAL_8N1, 20, 19); // Initialisierung der Hardware-Seriellen Schnittstelle 
  }
}
 String hexValue = "";
        ...
        if (irDataBuffer != "") {
          ...
          if (currentMillis - previousMillis >= interval) { //Das ist unnötig geworden.
            if (useWiFiMode == true) { //Könnte man eigentlich auch hier schreiben mit dem Verbinden, aber bringt so zeitlich einen Vorteil.
              ...
              WiFi.mode(WIFI_OFF);
              uint64_t scriptTimeSleep = (sleepTime - (millis() - scriptTimer));
